...
public class FuncionarioValidacao {
	...
	public static List<String> validaFuncionario(Funcionario funcionario){
		...
		// Valida para ver se tem numeros no campo Nome
		int flag = 0;
		for(int i=0 ; i<funcionario.getNome().length() ; i++) {
			char c = funcionario.getNome().charAt(i);
			if(Character.isDigit(c)) { flag++; }
		} if(flag != 0) {
			errosValidacao.add("* O Nome não deve conter números.");
		}
		...
		// Os campos Sexo e Plano de Saúde não necessitam de validação, 
		// pois são obrigatórios e seus valores são predefinidos:
		// Sexo: Masculino ou Feminino (serão usados dois componentes JRadioButton).
		// Plano de Saúde: Sim ou Não (será usado um componente JCheckBox).
