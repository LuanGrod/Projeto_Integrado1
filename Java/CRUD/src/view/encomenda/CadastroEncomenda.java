	...
	private void btnCadEncomenda() {
		...
		try {
			if (Integer.parseInt(tfQnt.getText()) == 0) {
				...
				erros = new EncomendaController().insereEncomenda((Produto) cbProduto.getSelectedItem(),
						(Fornecedor) cbFornecedor.getSelectedItem(), Integer.parseInt(tfQnt.getText()));
			}

			if (erros.size() == 0) { // Se o primeiro elemento do ArrayList for null.
				JOptionPane.showMessageDialog(this, "Encomenda cadastrada com sucesso.", "Informação",
