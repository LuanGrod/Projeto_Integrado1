...
public class ProdutoDao extends GenericDao {
	...
	public String insereProduto(Produto produto) {
		...
		return insere(instrucaoSql, produto.getNome(), produto.getPrecoCusto(), produto.getPrecoVenda(),
				produto.getQtdEstoque(), produto.getCategoria().getId());
	}
		...
		try {
			...
			if (excecao == null) {
				...
				if (registros.next()) { // Se for retornado pelo menos um registro.
					...
					while (registros.next()) {
						// Atribui o Id e a Descrição do cargo ao objeto Cargo por meio dos métodos set
						// e
				...
				// Libera os recursos usados pelo objeto Connection e fecha a conexão com o
				// banco de dados.
				ConnectionDatabase.getConexaoBd().close();
			}
		} catch (Exception e) {
		...
		try {
			...
			if (excecao == null) {
				...
				if (registros.next()) { // Se for retornado pelo menos um registro.
					registros.beforeFirst(); // Retorna o cursor para antes do 1º registro.
					while (registros.next()) {
						// Atribui os dados do funcionário ao objeto Funcionario por meio dos métodos
						// set e
						// adiciona este objeto ao ArrayList funcionarios.
						produto = new Produto();
	...
	public String alteraProduto(Produto produto) {
		instrucaoSql = "UPDATE PRODUTO SET Nome = ?, PrecoCusto = ?, PrecoVenda = ?, QntEstoque = ?, Categoria_idCategoria = ? "
				+ "WHERE idProduto = ?";
		return insere(instrucaoSql, produto.getNome(), produto.getPrecoCusto(), produto.getPrecoVenda(),
				produto.getQtdEstoque(), produto.getCategoria().getId(), produto.getId());
	}

	public void ListaDePesquisa(String tf) {
		TelaPrincipal.limpaPesquisa();
		instrucaoSql = "SELECT * FROM PRODUTO WHERE NOME LIKE '" + tf + "%' ORDER BY nome";
	    	 try {
		        	excecao = ConnectionDatabase.conectaBd(); // Abre a conexão com o banco de dados.
		        	if (excecao == null) {
		        		comando = ConnectionDatabase.getConexaoBd().prepareStatement(instrucaoSql);
						...
						while (registros.next()) {
							TelaPrincipal.insereProdutoLista(registros.getString("nome"));
						}
	...
	public Produto consultaProdutoByNome(String nome) {
		Produto produto = null;
		instrucaoSql = "SELECT * FROM PRODUTO WHERE nome = ?";
